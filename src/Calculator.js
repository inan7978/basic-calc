...
const buttons = ["+","-","*","/",7,8,9,4,5,6,1,2,3,"Neg",0,"."];

const mappedBtns = buttons.map((button) => {
    return <button className="keys" onClick={() => btnPressed(button)}>{button}</button>
})
        ...
        <div>
            <div className="container">
                <div className="screen">
                    <h5 className="prev-val">{prev}{operator}</h5>
                <h2 className="value">{value}</h2>
                </div>
            <div className="keys-container">
            {mappedBtns}
            <button className="keys all-clear" onClick={() => {
                setOperator("");
                setValue(0);
                setDecPres(false);
                setDecPlace(1);
            ...
            <button className="keys equal-sign" onClick={submitHandler}>=</button>
            </div>
            </div>
        </div>
    )
